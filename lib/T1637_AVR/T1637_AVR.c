...
void TM1637_writeByte(int8_t wr_data)// служебна€ функци€ записи данных по протоколу I2C, с подтверждением (ACK)
{
  ...
  DispPort &= ~(1<<Clkpin);
  _delay_us(5);
  DDRC &= ~(1<<Datapin);// если помен€ете порт на какой-то другой кроме PORTC, то тут тоже все DDRC на другие DDRx мен€ть надо будет
...
int main(void)// пример использовани€ вышеописанных функций

{
	uint8_t SegData1[4]={0,1,2,3};//записываем в массив цифры 0,1,2,3
	TM1637_init(1,5,4);//устанавливаем пины SCL и SDA
	TM1637_set(2,0,0xC0);// ставим €ркость 2
	_delay_ms(2000);//не знаю зачем, но тупим немного
    while(1) // а дальше в бесконечном цикле крутим в первом и четвертом разр€де цифры от 0 до F
    {
